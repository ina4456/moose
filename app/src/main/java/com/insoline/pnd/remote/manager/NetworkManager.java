package com.insoline.pnd.remote.manager;
    ...
    public synchronized void request(Context context, RequestPacket packet) {
        ...
        if ( packet instanceof LivePacket) {
            if (livePacketConnector == null || !livePacketConnector.isConnected()
                ...
                port = ConfigLoader.getInstance().getCallServerPort();
                livePacketConnect(AppDefine.IP_LIVE_PACKET, port);
            }
            ...
            if (connector == null || !connector.isConnected()
                    || !isAvailableNetwork(context)) {
                disconnect();
                ip = ConfigLoader.getInstance().getCallServerIp();
                port = ConfigLoader.getInstance().getCallServerPort();
                connect(ip, port);
            }
            if (connector != null) {
